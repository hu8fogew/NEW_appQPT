...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //日结
        self.payOfwaytext = [self addLabelWithTextColor:HWColor(31, 199, 193) andFont:13 andsView:topView];
        self.payOfwaytext.textAlignment = NSTextAlignmentCenter;
        self.payOfwaytext.layer.borderColor =[HWColor(31, 199, 193) CGColor];
        self.payOfwaytext.layer.borderWidth = 1;
        self.payOfwaytext.layer.cornerRadius = 3;
        self.payOfwaytext.layer.masksToBounds = YES;
        [self.payOfwaytext mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [self.salaryText mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.payOfwaytext.mas_left).with.offset(-5);
            make.centerY.equalTo(self.zwKindText);
        }];
        ...
        [self.numText mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.payOfwaytext);
            make.centerY.equalTo(self.mapImageView);
        }];
