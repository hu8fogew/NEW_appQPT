...
-(instancetype)initWithFrame:(CGRect)frame withDataArray:(NSArray *)dataArray withFont:(CGFloat)font {
    ...
    if (self) {
        ...
        self.lineColor = HWColor(0, 163, 219);
...
-(void)addSubSegmentView
{
    ...
    for (int i = 0 ; i < _dataArray.count ; i++) {
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(i * (width-5)+15, 0, width-30, self.frame.size.height)];
        button.tag = i+1;
...
//        [button setTitleColor:self.textSelectedColor forState:UIControlStateSelected];
        ...
        self.addBtn = [[UIButton alloc] initWithFrame:CGRectMake(5 * (width-5)+60, 10, 18, 18)];
        [self.addBtn setImage:[UIImage imageNamed:@"加号"] forState:UIControlStateNormal];
        ...
        if (i != _dataArray.count || i != 0) {
            UILabel *line = [[UILabel alloc ] initWithFrame:CGRectMake(i * width+15 , 0, 0.45, 40)];
            line.backgroundColor = [UIColor whiteColor];
            [self bringSubviewToFront:line];
            [self addSubview:line];
        }
    }
    self.lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(15, self.frame.size.height-2, width-30, 2)];
